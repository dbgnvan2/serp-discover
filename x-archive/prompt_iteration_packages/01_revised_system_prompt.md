# Revised Main Report System Prompt
...
### 1. Role redefinition: "auditor reporting findings" replaces "senior SEO analyst"
**Why:** The "senior analyst" framing invited the LLM to synthesize compelling narratives. The "auditor" framing constrains it to reporting what the data shows. This directly addresses the pattern where the LLM constructed a story around the trap recommendations and then found evidence to fit.

### 2. Tool recommendations demoted from primary evidence to appendix material
**Why:** The data dictionary now explicitly labels tool_recommendations_verified as "APPENDIX MATERIAL ONLY" and states they are "generated by a fixed template system, not by analysis of user intent." The report structure confines them to Section 6 assessment only, with an explicit statement that "this section does not drive the recommendations in Section 7." This is the single highest-leverage change — the traps were acting as the narrative spine of every failed report.

### 3. Section 2 restructured as per-keyword profiles before cluster synthesis
**Why:** The previous Section 2 let the LLM group keywords into clusters first, which caused it to apply one keyword's entity mix to the cluster. For example, calling the broad "estrangement" keyword "counselling-dominant" when it's actually legal-dominant (12 legal vs 5 counselling). Forcing per-keyword subsections first means the legal dominance is stated under "estrangement" specifically, and the synthesis paragraph must reference those stated facts.

### 4. Ten explicit evidence rules replace general principles
**Why:** The previous prompt had principles like "DATA OVER NARRATIVE" and "EVIDENCE DISCIPLINE" which were aspirational. The LLM agreed with them in spirit but violated them in practice because they didn't specify exactly what would constitute a violation. The ten rules are concrete and testable: "If a question is not in cross_cluster, it is NOT cross-cutting" leaves no room for interpretation. Each rule maps to a specific failure observed in previous reports.

### 5. Strategic flags elevated from reference to binding
**Why:** The previous prompt said "Follow the priority order in strategic_flags." The new prompt says "strategic flags are binding" and "these are computed facts, not suggestions." The ordering (defend before enter, skip means no recommendation) is now stated as a structural constraint rather than a preference.

### 6. Absent evidence handling made explicit (Rule 8)
**Why:** The most common fabrication pattern was the LLM filling gaps. When a keyword had no PAA questions, it borrowed from other keywords without disclosing this. When client AIO citations were zero, it described AIO "opportunities" as if the client currently had citations. Rule 8 requires stating absence directly rather than papering over it.

### 7. Mixed landscape rule (Rule 9) with specific threshold
**Why:** "Estrangement" has 12 legal and 5 counselling entities. Previous reports called it "legal-dominated" which is defensible, but the new rule sets a 60% threshold to prevent borderline cases from being described with false certainty. With 12/24 classified results (50%), estrangement would be described as "mixed with legal plurality" rather than "legal-dominated."

### 8. total_results labeling constraint (Rule 10)
**Why:** The advisory briefing converted total_results into "monthly searches" in a previous run. This rule prevents the pass-1 report from making the same error, which would then cascade into pass 2.

---

## Likely Remaining Failure Modes

1. **Bowen Theory over-application.** The client context describes the framework, and the LLM may describe every content gap as "perfectly suited for Bowen Theory" even when the PAA questions don't naturally invite a systems perspective. The rules constrain evidence but not interpretive enthusiasm about the framework's relevance.

2. **Competitor naming fabrication.** The rules constrain PAA question fabrication and trigger source attribution, but the LLM could still name competitors that don't appear in the top5_organic lists. The validation function doesn't currently check competitor names against the data.

3. **Autocomplete inference inflation.** Rule 4 allows citing autocomplete evidence for gaps without PAA questions, but the LLM could over-interpret a single autocomplete suggestion as strong demand signal. The rules require labeling it as "inferred from autocomplete" but don't prevent treating it as equivalent to PAA evidence in priority weighting.

4. **Section 6 leaking into Section 7.** Despite the structural separation, the LLM may still use the trap framework to color its Section 7 recommendations — e.g., framing a recommendation as "addressing the fusion trap" without using that label. The rules don't prevent thematic echo.

5. **Cluster synthesis contradicting per-keyword data.** The synthesis paragraph in Section 2 could smooth over keyword-level differences that the per-keyword subsections correctly stated. This is mitigated by requiring the synthesis to "reference the per-keyword data above" but not fully prevented.
