...
- **Per-brand sentiment + aspect keywords (`answer_sentiment.py`, Y.9).** OFF by default (`sentiment.enabled: false`, gate Y-D8): a disabled run makes **zero** sentiment calls, the report states "sentiment not measured", and AIVI excludes the axis (principle 3). When enabled, one LLM classification per answer that mentions the client or top competitor returns a polarity label + verbatim positive/negative aspect phrases (stored as a measured input); **Python computes `% positive`**. Each call counts against `ai_visibility.max_total_calls`. Persisted to `answer_sentiment`. The client's `% positive` is AIVI's Sentiment axis (or `None` → n/a when disabled/unmeasured).
